...
def Saisie_Notes(page, Donner):
    ...
    def get_teacher_subject():
        ...
        Matiere = supabase_db.get_teacher_subject(Donner.get("ident"))
        
        if not Matiere:
            Dialog.error_toast("Erreur not de la recuperation")
            return 
    ...
    def load_classes_with_students():
        ...
        Classe = supabase_db.get_classes_with_students(Etat)
        
        if not Classe:
            Dialog.error_toast("Erreur lors de la recuperation des classes")
            return
    ...
    def load_students_by_class(classe_nom):
        ...
        Student = supabase_db.get_students(Etat , classe_nom)
        if not Student:
            return []
        
        return [
            {
                s.get(key) for key in s.keys()
            } for s in Student
        ]
    ...
    def get_matiere_coefficient(matiere_nom):
        ...
        Etat = Return("etablissement")
        if not Etat:
            return "2"  # Coefficient par défaut
        
        #STEP 1 : Avoir tous les matiere de l'etablessement
        matiere = supabase_db.get_matieres(Etat)
        if not matiere:
            return "2"  # Coefficient par défaut
        for mat in matiere:
            #STEP 2 : Chercher la matiere voulue
            if mat.get("nom") == matiere_nom:
                return mat.get("coefficient" , "2")

    def check_note_exists(matricule, matiere, classe):
        """Vérifie si une note existe déjà pour cet élève dans cette matière"""
        
        #Pour recuperter a je vais utiliser la fonction get_note de superbase_db
        Note = supabase_db.get_note(matricule , matiere , classe)
        if Note:
            return (
                Note.get("classe"),
                Note.get("matricule"),
                ...
                Note.get("note_interrogation"),
                Note.get("note_devoir"),
                Note.get("note_composition"),
                Note.get("moyenne"),
                Note.get("date_saisie"),
            )
        return None
    
    def calculate_moyenne(note_interro, note_devoir, note_compo):
        """Calcule la moyenne selon le système de notation"""
        try:
            ...
            # Moyenne pondérée : (Interro + Devoir + 2*Compo) / 4
    ...
    def show_student_notes(student, classe_nom, matiere):
        ...
        if existing_note:
            # Notes déjà saisies - Mode Modification/Suppression
            show_existing_notes(student, classe_nom, matiere, existing_note)
        else:
            # Pas de notes - Mode Ajout
    ...
    def execute_delete_notes(confirm_dialog, parent_dialog, student, classe_nom, matiere):
        ...
        #con = None
        try:
            #Pour le supression de note je vais utiliser la fonction delete_note de supabase_db
            delect = supabase_db.delete_note(student[2] , matiere , classe_nom)
            if not delect:
                Dialog.error_toast("Erreur lors de la suppression des notes")
                return
    ...
    def show_add_notes_form(student, classe_nom, matiere, existing_note=None):
        ...
        def save_notes(matricule, classe, matiere, coef, interro, devoir, compo, is_update):
            ...
            try:
                ...
                if is_update:
                    # Mise à jour
                    #Pour la modfification je vais utiliser la fonction create_or_update_note de supabase_db
                       #-STEP 1 : Creation d'un dictionnaire avec les nouvelles valeurs
                    dic = {
                    "classe": classe,
                    "matricule": matricule,
                    "matiere": matiere,
                    "coefficient": coef,
                    "note_interrogation": interro,
                    "note_devoir": devoir,
                    "note_composition": compo,
                    "moyenne": moyenne,
                    "date_saisie": date_saisie
                    }
                        #-STEP 2 : Appel de la fonction create_or_update_note avec le dictionnaire
                    Modif = supabase_db.create_or_update_note(dic)
                    if not Modif:
                        Dialog.error_toast("Erreur lors de la modification des notes")
                        return
                    
                message = "Action effectué avec sucess !"
                # Dialog de succès
    ...
    def create_class_card(classe):
        ...
        # Compter combien de notes sont déjà saisies pour cette classe
        notes_count = 0
        if matiere:
            con = None
            try:
                Note_saisi = supabase_db.get_notes(classe = classe_nom , matiere = matiere)
                if not Note_saisi:
                    Dialog.error_toast("Erreur lors de la recuperation des notes")
                    notes_count = 0
                else:
                    #Mantenant nous allons triere en utilisant matricule car il se peux que Note_saisi 
                    #contient des note saisi  d'autre etablissement qui n'est pas ce que l'on cherche
                        #-STEP 1 : Recupperation de tous les matricule(eleve) des eleve de cet etablissement
                    Etudient = supabase_db.get_students(Return("etablissement") , classe_nom)
                    if not Etudient:
                        notes_count = 0
                    else:
                        matricule_list = [etud.get("matricule") for etud in Etudient]
                        #-STEP 2 : Filtrage des notes qui appartiennent a cet etablissement
                        filtered_notes = [note for note in Note_saisi if note.get("matricule") in matricule_list]
        ...
        return ft.Container(
            content=ft.Column([
                ...
                ft.Column([
                    ft.Text(f"{notes_count}/{effectif} notes saisies", size=12, color=ft.Colors.GREY_700),
                    ft.ProgressBar(
                    ...
                    ft.Text(f"{pourcentage}%", size=12, weight=ft.FontWeight.BOLD, color=progress_color),
                ],
                spacing=3,
                horizontal_alignment=ft.CrossAxisAlignment.CENTER,
                ),
